...
ansible-playbook -i hosts 05-k8s-master-sdn.yaml
ansible-playbook -i hosts 06-k8s-master-ingress.yaml
# recuperar token para add nodes al cluster
ansible-playbook -i hosts 07-k8s-worker-configuration.yaml
ansible-playbook -i hosts 08-k8s-cluster-usuario.yaml